...
- complete-guide
- second-brain
summary_en: A structured complete guide for planning the New Zealand pathway from
  Resident Visa to Permanent Resident Visa and citizenship by grant, covering start
  dates, PRV 2-year and 184-day rules, alternative PRV commitment paths, travel conditions,
  citizenship 240-day and 1,350-day presence rules, example timelines, algorithms,
  and common mistakes.
...
```mermaid
flowchart TD
    A[RV granted / resident status begins] --> B{Resident status at least 2 years?}
    B -- No --> B1[Usually not ready for PRV]
    B -- Yes --> C{Two prior 12-month blocks each have 184+ resident days in NZ?}
    C -- No --> C1[Delay PRV or consider another commitment route]
    C -- Yes --> D[Possible PRV application]
    D --> E[Continue accumulating resident days]
    E --> F{Past 5 years: each 12-month block has 240+ days and total is 1,350+ days?}
    F -- No --> F1[Keep accumulating qualifying presence]
    F -- Yes --> G[Possible citizenship by grant application]
```
